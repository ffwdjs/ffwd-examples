...
<!--[if gt IE 8]><!--> <html lang="<%= language  %>" class="no-js"> <!--<![endif]-->
  <head>
    ...
    <base href="/">
    <title><%- _.compact([docName, appName]).join('|')  %></title>
  ...
  <body>
        ...
        <section class="column medium-9">
          ...
          <div class="content">
            <div class="row">
              ...
              <% if (data.params) { %>
              <div class="column medium-6 params">
                <h5>Params</h5>
                
                <ul>
                  <% _.each(data.params, function(param) { %>
                  <li class="<%- (param.nullable ? 'nullable ' : '') + (param.variable ? 'variable ' : '') + (param.optional ? 'optional ' : '') %>">
                    <div class="name"><%- param.name %></div>
                    <% if (param.defaultValue) { %>
                    <div class="default-value"><%- param.defaultValue %></div>
                    <% } %>

                    <ul>
                      <% _.each(param.type.names, function(name) { %>
                      <li><a class="type" href="docs/<%- docSide %>/types/<%- name %>"><%- name %></a></li>
                      <% }); %>
                    </ul>
                  </li>
                  <% }); %>
              ...
              <% if (data.returns) { %>
              <div class="column medium-6 returns">
                <h5>Returns</h5>

                <ul>
                  <% _.each(data.returns, function(ret) { %>
                    <% _.each(ret.type.names, function(name) { %>
                    <li><a class="type" href="docs/<%- docSide %>/types/<%- name %>"><%- name %></a></li>
                    <% }); %>
                  <% }); %>
                </ul>
    ...
    <script>document.write('<script src="http://' + (location.host || 'localhost').split(':')[0] + ':'+ (parseInt(location.port || 80, 10) + 1) +'/livereload.js?snipver=1"></' + 'script>')</script>
  </body>
</html>
