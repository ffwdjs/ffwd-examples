  ...
  grunt.registerMultiTask('ffwd-doc', function() {
    ...
    for (name in deps) {
      ...
      if (grunt.file.exists(clientPath)) {
        grunt.verbose.writeln('will parse '+ name +' client at '+ clientPath);
        clientDeps.push(clientPath);
      }
      else {
        grunt.verbose.writeln('no scripts to parse for '+ name +' client at '+ clientPath);
      }
      
      if (grunt.file.exists(serverPath)) {
        grunt.verbose.writeln('will parse '+ name +' server at '+ serverPath);
        serverDeps.push(serverPath);
      }
      else {
        grunt.verbose.writeln('no scripts to parse for '+ name +' server at '+ serverPath);
      }
    }
